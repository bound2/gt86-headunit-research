...
### Bounded Lockdown service framing

`lockdown_wire.h` adds four-byte big-endian body-length framing and an explicit,
read-only GetValue XML encoder. `lockdown_channel.h` binds one plain dispatcher
stream, copies one request and receives exactly one bounded response. It
preserves coalesced trailing bytes for explicit handoff, requires the request
to be physically sent and TCP-acknowledged, and protects held responses with
release tokens and deadlines. Channel state is 160 host bytes plus caller buffers.
...
`service_plist.h` now decodes a bounded XML/binary plist subset into caller-owned
nodes/bytes. It rejects duplicate keys, invalid Unicode, reference cycles and
capacity violations. `lockdown_reply.h` validates expected response types, errors,
service ports and TLS flags; its explicit bridge reads held channel responses
without releasing them or doing I/O. Thirteen new groups and seven binary
...
The separate projection-control path now has `rtsp_wire.h`/`rtsp_channel.h`:
bounded RTSP/HTTP framing, binary bodies, explicit token-bound responses and
absolute receive/reply/output deadlines. It preserves unread bytes across
request boundaries and waits for explicit output release before receiving
another message, allowing a future verified cipher handoff. Nine test groups
...
still missing. Nine output groups, Windows COM/clock argument checks and real
...
enable codecs. Seven decoder and seven actual-UDP integration groups pass, with
...
Explicit relative pacing and bounded loss recovery now connect the codec bridge
to timed PCM startup. Opus uses actual PLC; PCM/AAC gaps use marked silence, with
...
packets and a late replay using a synthetic progressing device. Sender-clock
mapping, A/V sync and physical validation remain work.
...
Thirteen PCM groups and delayed AAC/Opus bursts over IPv4/IPv6 pass with a synthetic
...
Optional `drift_ppm` now enables bounded per-stream WASAPI rate adjustment from
observed device-clock samples. Eight accelerated encrypted AAC/Opus streams test
fast/slow clocks without dropping PCM or restarting playback. Correction is
against the local schedule, not the phone clock; the real endpoint/resampler
...
The [iAP consumer/profile follow-up](reports/iap-stream-profile.md) distinguishes
wireless link-framed tunnel sessions from existing wired carkit ownership. Both
reference paths use version-2 control and zero link ACK/retry fields; an explicit
opt-in now supports that profile through the existing carkit/TLS bridge without
changing defaults. There are 21 link and seven carkit integration groups. Actual
wired AV still needs the separate USB-network interface and native factory backend.

The [factory wired-network trace](reports/factory-usb-network.md) finds a shipped
NCM driver, its startup references and selected USB descriptor/parameter paths.
The selected factory IPv4 stack has no built-in IPv6 domain; native IPv6 and
...
The [native build/runtime audit](reports/factory-native-build-route.md) reads all
seven nested ZIPs without finding an ELF stack or SDK. A pinned public ARMv7
toolchain candidate contains an older IPv6 stack, but it lacks two exports used
