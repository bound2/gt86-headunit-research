...
int projection_decode_create(uint32_t bit,uint64_t gen,projection_decode **out) {
    ...
    if(format.codec==PROJECTION_AUDIO_OPUS) {
        ...
        s->aac=NeAACDecOpen(); if(!s->aac) { projection_decode_destroy(s); return IAP2_NO_SPACE; }
        config=NeAACDecGetCurrentConfiguration(s->aac);
        if(!config) { projection_decode_destroy(s); return IAP2_PROVIDER_FAILED; }
        config->defObjectType=LC; config->defSampleRate=format.clock_rate; config->outputFormat=FAAD_FMT_16BIT;
        config->downMatrix=0; config->dontUpSampleImplicitSBR=1; config->useOldADTSFormat=0;
        asc[0]=(uint8_t)(format.aac_config>>8); asc[1]=(uint8_t)format.aac_config;
        if(!NeAACDecSetConfiguration(s->aac,config)||NeAACDecInit2(s->aac,asc,2,&rate,&channels)!=0||rate!=format.clock_rate||channels!=format.channels) {
            projection_decode_destroy(s); return IAP2_UNSUPPORTED;
        }
    }
...
int projection_decode_packet(projection_decode *s,uint64_t gen,const projection_audio_packet *p,projection_decoded_audio *out) {
    uint32_t frames=0,duration=0; size_t i; uint8_t priming=0;
    ...
    if(s->format.codec==PROJECTION_AUDIO_PCM16) {
        ...
        if(info.error||decoded!=s->pcm||buffer!=s->pcm||info.bytesconsumed!=p->size||info.channels!=2||info.samplerate!=s->format.clock_rate||
           ...
           (info.samples!=2048&&(s->received||info.samples!=0))) return fail(s,IAP2_INVALID);
    ...
    out->channels=s->format.channels; out->priming=priming; return IAP2_OK;
}
