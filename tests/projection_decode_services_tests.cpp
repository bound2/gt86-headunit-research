...
#include <memory>
struct Pipeline {
    uint64_t now=ms; unsigned closed=0; bool automatic=false; int native_ppm=0;
    struct Device final:projection_pcm::Device {
        ...
        uint64_t tick_ns=0; double exact_frames=0; int32_t ppm=0; unsigned changes=0;
        Device(Pipeline& p,const projection_audio_format& f):owner(p),format(f) { capacity=6000; frequency=uint64_t(f.clock_rate)*4; period_ns=ms; rate_adjustable=true; }
        ~Device() noexcept override { ++owner.closed; }
        void tick() noexcept { if(owner.automatic&&running) {
            exact_frames+=double(owner.now-tick_ns)*format.clock_rate/1000000000.0*(1.0+owner.native_ppm/1000000.0)*(1.0+ppm/1000000.0); tick_ns=owner.now;
            uint64_t frames=static_cast<uint64_t>(exact_frames);
        ...
        int start() noexcept override { ++starts; running=true; tick_ns=start_ns=owner.now; return IAP2_OK; }
        int reset() noexcept override { pad=0; pos=released=0; running=false; exact_frames=0; return IAP2_OK; }
        int position(uint64_t& p,uint64_t& q) noexcept override { tick(); p=pos; q=owner.now; return IAP2_OK; }
        int adjust(int32_t value) noexcept override { tick(); ppm=value; ++changes; return IAP2_OK; }
    };
...
static void delayed_burst(const Vectors& v) {
    for(bool ipv6:{false,true}) for(bool aac:{false,true}) {
        auto owner=std::make_unique<Pipeline>(ipv6,true,30); auto& f=*owner; Socket phone(SOCK_DGRAM,ipv6);
        uint32_t bit=aac?0x400000:0x10000000,duration=aac?1024:960,rate=aac?44100:48000,first=UINT32_MAX-999;
        const unsigned count=100; auto e=f.open(100,bit,100); auto d=f.devices[0]; auto p=projection_audio_services_provider(&f.audio);
        f.start(e.lease); uint64_t beginning=f.now; unsigned sent=0,after=0; uint8_t key[32]; std::fill(key,key+32,9);
        std::vector<Bytes> packets; for(unsigned i=0;i<count;++i) packets.push_back(v.at(std::string(aac?"aac44100_":"opus20_")+std::to_string(i%5)));
        auto packet=[&](unsigned i) { return audio_packet(key,i,first+i*duration,packets[i]); };
        for(uint64_t elapsed=0;elapsed<=uint64_t(count)*duration*1000000000/rate+250*ms;elapsed+=ms) {
            f.now=beginning+elapsed;
            unsigned gate=sent==20?40:sent;
            if(sent<count&&elapsed>=uint64_t(gate)*duration*1000000000/rate) {
                for(unsigned end=std::min(count,sent+(sent==20?25:5));sent<end;++sent) {
                    phone.datagram(packet(sent),e.data_port);
                    f.until([&]{return f.audio.slots[0].audio.received&&f.audio.slots[0].audio.highest==sent;});
                }
            }
            CHECK(f.poll()==IAP2_OK); projection_playback_position pos{};
            CHECK(p.playback(p.context,91,e.lease,&pos)==IAP2_OK);
            if(pos.has_position) {
                uint32_t frame=pos.sample_time-first; CHECK(frame<count*duration);
                uint64_t due=beginning+100*ms+uint64_t(frame)*1000000000/rate;
                CHECK(pos.raw_ns<=due+30*ms+1000000000/rate+1);
                if(frame>=50*duration) ++after;
            }
        }
        auto pcm=expected(bit,packets,first,duration); size_t tail=20*duration*(aac?4:2);
        CHECK(sent==count&&d->starts>=2&&d->starts<=4&&after>500);
        CHECK(d->data.size()<pcm.size()&&d->data.size()>pcm.size()/2&&d->data.size()>=tail);
        CHECK(std::equal(d->data.end()-tail,d->data.end(),pcm.end()-tail)); // Dropping playback never resets codec history.
        auto& stream=f.audio.slots[0]; CHECK(!stream.audio.count&&stream.audio.highest==count-1&&stream.peer_port==phone.port);
        auto seen=stream.audio.seen; phone.datagram(packet(20),e.data_port); CHECK(f.poll()==IAP2_OK&&stream.audio.highest==count-1&&stream.audio.seen==seen);
        CHECK(Bytes(stream.audio.key,stream.audio.key+32)==Bytes(key,key+32));
        p.close(p.context,91,e.lease); CHECK(f.closed==1&&!f.audio.wsa); Socket reuse(SOCK_DGRAM,ipv6,1,e.data_port);
    }
}
static void drifting_device(const Vectors& v) {
    for(bool ipv6:{false,true}) for(bool aac:{false,true}) for(int native:{-300,300}) {
        auto owner=std::make_unique<Pipeline>(ipv6,true,30,1000); auto& f=*owner; f.native_ppm=native; Socket phone(SOCK_DGRAM,ipv6);
        uint32_t bit=aac?0x400000:0x10000000,duration=aac?1024:960,rate=aac?44100:48000,first=UINT32_MAX-999;
        const unsigned count=2000; auto e=f.open(100,bit,100); auto d=f.devices[0]; auto p=projection_audio_services_provider(&f.audio);
        f.start(e.lease); uint64_t beginning=f.now; unsigned sent=0,observations=0; int32_t corrected=0; uint64_t worst=0;
        uint8_t key[32]; std::fill(key,key+32,9);
        for(uint64_t elapsed=0;elapsed<=uint64_t(count)*duration*1000000000/rate+250*ms;elapsed+=ms) {
            f.now=beginning+elapsed;
            if(sent<count&&elapsed>=uint64_t(sent)*duration*1000000000/rate) {
                for(unsigned end=std::min(count,sent+5);sent<end;++sent) {
                    if(sent==717) continue; // Recovery PCM still has an actual device clock, but no source feedback anchor.
                    phone.datagram(audio_packet(key,sent,first+sent*duration,v.at(std::string(aac?"aac44100_":"opus20_")+std::to_string(sent%5))),e.data_port);
                    f.until([&]{return f.audio.slots[0].audio.received&&f.audio.slots[0].audio.highest==sent;});
                }
            }
            CHECK(f.poll()==IAP2_OK); projection_playback_position pos{}; CHECK(p.playback(p.context,91,e.lease,&pos)==IAP2_OK);
            CHECK(std::abs(d->ppm)<=1000);
            if(pos.has_position&&elapsed>30000*ms) {
                uint32_t frame=pos.sample_time-first; CHECK(frame<count*duration);
                uint64_t due=beginning+100*ms+uint64_t(frame)*1000000000/rate;
                worst=std::max(worst,pos.raw_ns>due?pos.raw_ns-due:due-pos.raw_ns); ++observations; corrected=d->ppm;
            }
        }
        CHECK(observations>9000&&worst<1000000&&std::abs(corrected+native)<120);
        CHECK(d->starts==1&&d->changes>5&&d->ppm==0); // Normal final drain restores nominal rate.
        CHECK(d->data.size()==size_t(count-(aac?1:0))*duration*(aac?4:2)); // No dropped/duplicated PCM bytes.
        CHECK(!f.audio.slots[0].audio.count&&f.audio.slots[0].audio.highest==count-1&&f.audio.slots[0].peer_port==phone.port);
        p.close(p.context,91,e.lease); CHECK(f.closed==1&&!f.audio.wsa); Socket reuse(SOCK_DGRAM,ipv6,1,e.data_port);
    }
}
