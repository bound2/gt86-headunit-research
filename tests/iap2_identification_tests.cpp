...
static void maximum_and_capacity() {
    ...
    const auto b = encode(m); check(b.size() == 930, "bounded maximum encoded size with one packed language parameter");
    ...
    Bytes packed;
    for (const auto& language : languages) { packed.insert(packed.end(), language.begin(), language.end()); packed.push_back(0); }
    check(decoded.at(9) == std::vector<Bytes>{hex("ffff")} && decoded.at(13) == std::vector<Bytes>{packed},
          "current/language encoding boundaries and one packed list field");
    for (size_t capacity = 0; capacity < b.size(); ++capacity) {
        out.assign(b.size() + 2, 0xa5); n = 777;
        check(iap2_identification_encode(&m, out.data() + 1, capacity, &n) == IAP2_NO_SPACE && !n &&
              out == Bytes(b.size() + 2, 0xa5), "every undersized capacity leaves packed-list output unchanged");
    }
}
static void packed_supported_languages() {
    // Independent expected payloads from the pinned [list str] encoding rule,
    // not generated by the implementation under test or a permissive decoder.
    const std::array<const char *, 4> names{"en", "de", "et", "fi"};
    const std::array<const char *, 4> golden{"656e00", "656e00646500", "656e00646500657400", "656e00646500657400666900"};
    for (size_t count = 1; count <= names.size(); ++count) {
        auto m = metadata(); m.language_count = count;
        for (size_t i = 0; i < count; ++i) m.languages[i] = text(names[i]);
        m.current_language = m.languages[count - 1];
        const auto actual = fields(encode(m));
        check(actual.at(13) == std::vector<Bytes>{hex(golden[count - 1])}, "languages share one NUL-packed parameter 13");
        Bytes current(names[count - 1], names[count - 1] + 2); current.push_back(0);
        check(actual.at(12) == std::vector<Bytes>{current}, "current language remains an independent scalar string");
    }
}
static void sequence_and_snapshot() {
    auto m = metadata(); std::string source = "Synthetic test", language = "et"; m.name = {source.data(), source.size()};
    m.language_count = 2; m.languages[1] = {language.data(), language.size()};
    const auto expected = encode(m); iap2_identification id{}; size_t n = 777; Bytes out(10, 0xa5);
    check(iap2_identification_init(&id, &m) == IAP2_OK, "snapshot init"); source.assign(source.size(), 'z'); language.assign(2, 'z');
