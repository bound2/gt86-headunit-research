...
| Supply the wired AV network | [Step 96](factory-usb-network.md) finds native NCM and no built-in IPv6 domain; [Step 97](factory-ncm-lifecycle.md) traces selected event handling and finds no named IPv6 stack in the outer ISO directory; [Step 98](factory-usb-ownership.md) makes separate interface ownership a candidate, not verified coexistence | Phone-associated interface and stock-service coordination, compatible IPv6 runtime, scoped address/listeners and lifetime ownership |
