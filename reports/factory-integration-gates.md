...
| Build a factory process | [Step 100](factory-native-build-route.md) identifies ARMle-v7/SP1; [Step 101](qnx-network-diagnostic.md) adds native diagnostic source and an SDK-only builder, but actual preflight stops on missing SDK paths; host model tests do not compile the SDK branch | Usable compatible QNX development inputs, actual native build and target execution/recovery |
| Own the phone transport | [USB trace](usb-transport.md) identifies stock HID/media/audio service ownership; [Step 98](factory-usb-ownership.md) verifies selected interface-level conflict checks, removal deferral and callback/detach lifetime constraints | Native interface access, actual cancellation retirement, device-wide configuration coordination and an ownership/restore plan verified on the unit |
| Supply the wired AV network | [Step 96](factory-usb-network.md) finds NCM but no built-in IPv6 domain; [Step 98](factory-usb-ownership.md) verifies selected interface-claim constraints; [Step 99](factory-network-identity.md) traces name/index caching; [Step 100](factory-native-build-route.md) finds no ELF stack in seven nested ZIPs and two missing NCM exports in a public older IPv6-stack candidate | Phone-associated interface and stock-service coordination, compatible IPv6 runtime, current scoped addresses, owned listeners and reconnect invalidation |
