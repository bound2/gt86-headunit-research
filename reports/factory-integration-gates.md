...
| Supply the wired AV network | [Step 96](factory-usb-network.md) finds NCM but no built-in IPv6 domain; [Step 97](factory-ncm-lifecycle.md) finds no named IPv6 stack in the outer ISO; [Step 98](factory-usb-ownership.md) verifies selected interface-claim constraints; [Step 99](factory-network-identity.md) establishes name/index caching and an IPv6-aware client, not a USB/phone join or IPv6 stack | Phone-associated interface and stock-service coordination, compatible IPv6 runtime, current scoped addresses, owned listeners and reconnect invalidation |
