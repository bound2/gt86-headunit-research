...
| Supply the wired AV network | [Step 96](factory-usb-network.md) finds native NCM and no built-in IPv6 domain; [Step 97](factory-ncm-lifecycle.md) verifies selected event handling, traces ownership/abort caveats and finds no named IPv6 stack in the outer ISO directory | Actual USB claim/cancellation guarantees, phone-associated interface, compatible IPv6 runtime, scoped address/listeners and lifetime ownership |
