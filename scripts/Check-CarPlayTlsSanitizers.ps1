...
if ($IncludeEnrollment) {
    ...
    foreach ($headunitName in @('pair_crypto','pair_verify','control_cipher','projection_control','pair_srp','pair_setup','pair_setup_channel','pair_store','pair_store_file','mfi_sap','projection_auth','projection_receiver')) {
        $headunitSources += Join-Path $headunitRoot "src/carplay/$headunitName.c"
    }
}
$headunitObjects = @()
...
try {
    ...
    if ($IncludeEnrollment) { $headunitTests += @('pair_setup_tests','pair_store_file_tests','mfi_sap_tests','projection_receiver_tests') }
