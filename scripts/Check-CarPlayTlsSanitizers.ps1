...
if ($IncludeEnrollment) {
    ...
    foreach ($headunitName in @('pair_crypto','pair_verify','control_cipher','projection_control','pair_srp','pair_setup','pair_setup_channel','pair_store','pair_store_file','mfi_sap','projection_auth','projection_session','projection_receiver','projection_services_win','projection_audio','projection_audio_services_win')) {
        $headunitSources += Join-Path $headunitRoot "src/carplay/$headunitName.c"
    }
}
$headunitObjects = @()
...
try {
    ...
    foreach ($headunitTest in $headunitTests) {
        ...
        if ($headunitTest -in @('mfi_sap_tests','projection_receiver_tests','projection_services_tests','projection_audio_services_tests')) { $headunitFixture = 'tests/fixtures/mfi-sap-vectors.txt' }
        if ($headunitTest -eq 'projection_audio_tests') { $headunitFixture = 'tests/fixtures/projection-audio-vectors.txt' }
