...
foreach ($headunitName in @('iap2_wire', 'iap2_auth', 'iap2_link', 'iap2_control', 'iap2_identification', 'iap2_transport',
    ...
    'lockdown_wire', 'lockdown_channel', 'service_plist', 'lockdown_reply', 'lockdown_bootstrap', 'rtsp_wire', 'rtsp_channel', 'pair_tlv', 'projection_info', 'lockdown_tls', 'lockdown_client', 'carkit', 'carkit_iap2')) {
...
if ($IncludeEnrollment) {
    ...
    foreach ($headunitName in @('pair_crypto','pair_verify','control_cipher','projection_control','pair_srp','pair_setup','pair_setup_channel','pair_store','pair_store_file','mfi_sap','projection_auth','projection_session','projection_receiver')) {
        $headunitSources += Join-Path $headunitRoot "src/carplay/$headunitName.c"
    }
}
$headunitObjects = @()
...
try {
    ...
    if ($IncludeEnrollment) { $headunitTests += @('pair_setup_tests','pair_store_file_tests','mfi_sap_tests','projection_session_tests','projection_receiver_tests') }
    foreach ($headunitTest in $headunitTests) {
        ...
        if ($headunitTest -in @('mfi_sap_tests','projection_receiver_tests')) { $headunitFixture = 'tests/fixtures/mfi-sap-vectors.txt' }
        ...
        if ($headunitTest -in @('mfi_sap_tests','projection_receiver_tests')) { $headunitTestArguments += (Join-Path $headunitRoot 'tests/fixtures/pair-setup-vectors.txt') }
        if ($headunitTest -eq 'projection_receiver_tests') { $headunitTestArguments += (Join-Path $headunitRoot 'tests/fixtures/projection-session-vectors.txt') }
