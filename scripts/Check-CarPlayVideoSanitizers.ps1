...
try {
    ...
    cmake --build build/video-service-sanitized --target projection_h264_tests projection_video_tests projection_video_limit_tests projection_video_services_tests
    if ($LASTEXITCODE -ne 0) { throw 'Sanitized video build failed' }
    ctest --test-dir build/video-service-sanitized -R '^projection_(h264|video|video_limit|video_services)_tests$' --output-on-failure
...
Write-Output 'PASS: video TCP/session/input/decoder adapters, tests, Mbed TLS, Monocypher and generic OpenH264 instrumented with ASan/UBSan; no target execution.'
