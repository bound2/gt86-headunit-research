...
try {
    ...
    cmake --build build/video-service-sanitized --target projection_h264_tests projection_h264_source_tests projection_h264_source_limit_tests projection_video_tests projection_video_limit_tests projection_video_services_tests projection_video_pixels_tests projection_video_gdi_tests projection_iap_tests projection_iap_services_tests
    if ($LASTEXITCODE -ne 0) { throw 'Sanitized video build failed' }
    ctest --test-dir build/video-service-sanitized -R '^projection_(h264|h264_source|h264_source_limit|video|video_limit|video_services|video_pixels|video_gdi|iap|iap_services)_tests$' --output-on-failure
...
Write-Output 'PASS: video TCP/session/input/decoder/pixel/GDI and iAP TCP/input/relay adapters, tests, Mbed TLS, Monocypher and generic OpenH264 instrumented with ASan/UBSan; Windows DLLs excluded; no vehicle execution.'
