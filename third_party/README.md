...
## Session-owned iAP TCP and explicit relay (Step 94)

The local Windows service, C API, tests and three synthetic binary-plist session
fixtures select GPL-3.0-only. They compose existing input/session/crypto owners;
no new upstream implementation body is copied or dependency downloaded. The
existing pinned LIVI tunnel/session files supply receive/return-path observations.
The reference opens its helper relay at RECORD; the local adapter's permission
to receive before RECORD requires an explicitly prepared route, not an inferred
upstream lifecycle guarantee.

Microsoft's Winsock documentation supplies accept/listener and exclusive-binding
semantics. IP Helper is test-only for OS-observed listener state. The independent
checker reuses PyCA 50.0.1; the new fixtures are generated by Python plistlib,
not phone captures or vendor data. Existing dependency notices remain unchanged.
